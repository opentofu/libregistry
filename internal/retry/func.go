...
func Func(
	...
	for {
		...
		if !isRetryable(err) {
			...
			return fmt.Errorf("non-retryable error encountered while attempting to %s, aborting (%w)", description, err)
		}
		tries++
		if tries >= maxTries {
			logger.Trace(ctx, "Max tries exhausted while attempting to %s, aborting (last error was: %v)", description, err)
			return fmt.Errorf("max tries exhausted while attempting to %s, aborting (last error was: %w)", description, err)
		}
		...
		case <-ctx.Done():
			...
			return fmt.Errorf("timeout while attempting to %s (last error was: %w)", description, err)

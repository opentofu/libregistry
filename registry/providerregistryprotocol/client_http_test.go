...
package providerregistryprotocol_test
...
func TestHTTPClient(t *testing.T) {
	cli, err := providerregistryprotocol.NewClient()
	if err != nil {
		t.Fatalf("Failed to create client (%v)", err)
	}
	ctx := context.Background()

	awsResponse, err := cli.ListAvailableProviderVersions(ctx, providerregistryprotocol.ListAvailableProviderVersionsRequest{
		Namespace: "opentofu",
		Type:      "aws",
	})
	if err != nil {
	...
	version, err := cli.GetProviderVersion(ctx, providerregistryprotocol.GetProviderVersionRequest{
		Namespace: "opentofu",
		Type:      "aws",
		Version:   "5.65.0",
		OS:        "darwin",
